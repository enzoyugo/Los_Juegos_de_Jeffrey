...
## Isolated semantic solver V2 GLB load check. Does not touch battle.

const OUTPUT := "res://docs/generated/SEMANTIC_SOLVER_V2_GODOT_LAB_VALIDATION.txt"

const MODELS := {
	"terere": {
		"glb": "res://assets/fighters/processed/semantic_solver_v2/terere/terere_idle_semantic_v2.glb",
		"scene": "res://scenes/debug/TerereSemanticSolverV2Lab.tscn",
	},
	"jaguarete": {
		"glb": "res://assets/fighters/processed/semantic_solver_v2/jaguarete/jaguarete_idle_semantic_v2.glb",
		"scene": "res://scenes/debug/JaguareteSemanticSolverV2Lab.tscn",
	},
}


func _initialize() -> void:
	var lines: PackedStringArray = PackedStringArray(["=== ActorCore Semantic Idle Solver V2 Godot Lab ==="])
	var ok := true
	for fighter_id in MODELS:
		if not _validate(fighter_id, MODELS[fighter_id], lines):
			ok = false
	_write(OUTPUT, lines)
	print("SEMANTIC_SOLVER_V2_GODOT_LAB=%s" % ("PASS" if ok else "FAIL"))
	quit(0 if ok else 1)
...
func _validate(fighter_id: String, cfg: Dictionary, lines: PackedStringArray) -> bool:
	...
	if player != null:
		...
		for anim_name in player.get_animation_list():
			...
			if idle_name.is_empty() and lower.contains("idle"):
				idle_name = anim_name
			if rest_name.is_empty() and lower.contains("rest"):
				rest_name = anim_name
			if standing_name.is_empty() and lower.contains("standing"):
				standing_name = anim_name
	...
	lines.append("rest=%s" % rest_name)
	lines.append("canonical_standing=%s" % standing_name)
	...
	var passed := (
		skeletons.size() == 1
		and bone_count == 101
		and not idle_name.is_empty()
		and not rest_name.is_empty()
		and not standing_name.is_empty()
		and bone_tracks >= 4
	)
