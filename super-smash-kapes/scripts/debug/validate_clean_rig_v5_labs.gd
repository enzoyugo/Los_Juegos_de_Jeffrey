...
const OUT_PATH := "res://docs/generated/CLEAN_RIG_V5_GODOT_LAB_VALIDATION.json"
const LABS := [
	"res://scenes/debug/TerereCleanRigV5Lab.tscn",
	"res://scenes/debug/JaguareteCleanRigV5Lab.tscn",
]
...
func _run() -> void:
	...
	for path in LABS:
		...
		if inst.has_method("collect_dump"):
			...
			row["ok"] = bool(dump.get("load_ok", false)) and int(dump.get("skeleton_count", 0)) == 1 and int(dump.get("bone_count", 0)) >= 80 and not bool(dump.get("fallback", true)) and upright and int(dump.get("animation_count", -1)) == 0
			if str(dump.get("pipeline", "")) != "V5":
				row["ok"] = false
				row["error"] = "wrong_pipeline"
			if str(dump.get("asset", "")).find("clean_rig_v5") == -1:
				row["ok"] = false
				row["error"] = "wrong_asset"
			if str(dump.get("asset", "")).find("game_ready") != -1:
				row["ok"] = false
				row["error"] = "v4_dependency"
			if str(dump.get("asset", "")).find("source_rigged") != -1:
				row["ok"] = false
				row["error"] = "source_fbx_dependency"
		else:
	...
	if fh:
		...
		fh.close()
	print("CLEAN_RIG_V5_LAB_VALIDATION_WRITTEN all_ok=%s path=%s" % [report["all_ok"], abs_path])
