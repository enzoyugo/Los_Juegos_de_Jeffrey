...
const OUT_PATH := "res://docs/generated/SEMANTIC_IDLE_POLISH_V1_GODOT.json"
const LABS := [
	"res://scenes/debug/TerereSemanticIdlePolishV1Lab.tscn",
	"res://scenes/debug/JaguareteSemanticIdlePolishV1Lab.tscn",
]
const FORBIDDEN := [
	"game_ready_v4",
	"game_ready_v3",
...
func _run() -> void:
	...
	for path in LABS:
		...
		if inst.has_method("collect_dump"):
			...
			row["dump"] = dump
			row["ok"] = bool(dump.get("load_ok", false)) and int(dump.get("bone_count", 0)) >= 90 and not bool(dump.get("fallback", true))
			if str(dump.get("pipeline", "")) != "SEMANTIC_IDLE_POLISH_V1":
				row["ok"] = false
	...
	if fh:
		...
		fh.close()
	print("SEMANTIC_IDLE_POLISH_LAB_VALIDATION_WRITTEN all_ok=%s path=%s" % [report["all_ok"], abs_path])
