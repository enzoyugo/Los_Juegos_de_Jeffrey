...
## SDS V4.3 facade-only review lab. Same candidate GLB, tighter architecture cameras.
...
func _ready() -> void:
	...
	Probe.dump("sds_v4_3_facade_only_lab", self)
...
func _load_env() -> void:
	if not ResourceLoader.exists(ENV_V43):
		print("[SDS_V4_3_FACADE_ONLY_LAB] MISSING glb")
		return
	var packed: PackedScene = load(ENV_V43) as PackedScene
	if packed == null:
		print("[SDS_V4_3_FACADE_ONLY_LAB] FAIL instantiate")
		return
	var inst := packed.instantiate()
	inst.name = "BlenderEnvV4_3FacadeOnly"
	add_child(inst)
	_strip(inst)
	print("[SDS_V4_3_FACADE_ONLY_LAB] loaded=%s HUMAN_REVIEW_REQUIRED canonical=false" % ENV_V43)
