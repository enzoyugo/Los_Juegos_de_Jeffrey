...
## SDS V4.2 facade cohesion CANDIDATE. Does not replace V3/V4/V4.1.
## HUMAN_REVIEW_REQUIRED. Not SDS_V4_2_CANONICAL.
## Cameras stay on facade_front until hold elapses, then advance one named view per interval.

const ENV_V42 := "res://assets/environments/shopping_del_sol/processed/shopping_del_sol_zombies_environment_v4_2_candidate.glb"
...
func _ready() -> void:
	...
	Probe.dump("sds_v4_2_candidate_lab", self)
...
func _process(delta: float) -> void:
	...
	var i := 0
	if _cycle > 1.2:
		i = int((_cycle - 1.2) / 3.2) % VIEW_ORDER.size()
	_apply(VIEW_ORDER[i])
...
func _load_env() -> void:
	if not ResourceLoader.exists(ENV_V42):
		print("[SDS_V4_2_CANDIDATE_LAB] MISSING glb")
		return
	var packed: PackedScene = load(ENV_V42) as PackedScene
	if packed == null:
		print("[SDS_V4_2_CANDIDATE_LAB] FAIL instantiate")
		return
	var inst := packed.instantiate()
	inst.name = "BlenderEnvV4_2Candidate"
	add_child(inst)
	_strip(inst)
	print("[SDS_V4_2_CANDIDATE_LAB] loaded=%s HUMAN_REVIEW_REQUIRED canonical=false" % ENV_V42)
