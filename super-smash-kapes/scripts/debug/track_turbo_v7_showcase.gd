...
## Human-facing Track Turbo V7 vertical slice. Debug HUD off by default.
## Does not replace TrackMain. VISUAL_REVIEW_PENDING.
...
func _ready() -> void:
	...
	], 24.0, Config.FUEL_MULTIPLIER)
	_reveal_cam = Camera3D.new()
	_reveal_cam.fov = 68.0
	add_child(_reveal_cam)
	_chase = CamScript.new()
	add_child(_chase)
	_set_state(ST_SETUP)
	print("[TRACK_TURBO_V7] 1 SHORT 2 MEDIUM 3 LONG  T difficulty  ENTER dale  F3 debug  F8 skip reveal")
		...
		ST_CARD:
			...
			if _card_t <= 0.0:
				if str(_hs.phase) == Hotseat.PHASE_DONE:
					_hud.set_banner("FIN DE MESA", true)
...
func _generate_and_reveal() -> void:
	...
	var idxs := Checkpoints.plan(seq, _length)
	_gates = Checkpoints.build_gates(self, _pieces, idxs, Config.ROAD_WIDTH)
...
func _on_reveal_done() -> void:
	...
	_hs.setup([
		...
		{"id": "santi", "name": "Santi", "color": Color("#e87a4a")},
		{"id": "tomi", "name": "Tomi", "color": Color("#7ad07a")},
	], expected, Config.FUEL_MULTIPLIER)
...
func _on_cp(body: Node, gate: Dictionary) -> void:
	...
	if split_tgt >= 0.0:
		_hud.set_delta(_timer - split_tgt)
	else:
		_hud.clear_delta()
	_next_cp += 1
...
func _finish() -> void:
	...
	_hud.set_banner(str(rec.get("card", "")), true)
	_card_t = 2.2
	_set_state(ST_CARD)
...
func _target_ms(p: Dictionary) -> int:
	var mine: int = int(p.get("best_ms", -1))
	var worst := -1
	var second := -1
	for row in _hs.ranking():
		var ms: int = int(row.get("best_ms", -1))
		if ms < 0:
			continue
		if worst < 0 or ms > worst:
			second = worst
			worst = ms
		elif ms > second:
			second = ms
	if mine == worst and second > 0:
		return second
	return worst
...
func _refresh_setup() -> void:
	if _state != ST_SETUP:
		return
	_hud.set_banner("TRACK TURBO\n%s  %s\nSEED %d\n\nENTER GENERAR" % [_length, _diff, _seed], true)


func _refresh_hud_board() -> void:
	_hud.set_ranking(_hs.ranking(), _hs.current_id)
	if _debug:
		_hud.set_debug("fov=%.1f speed=%.1f drift=%s cp=%d/%d" % [
...
func _sun() -> void:
	...
	env.background_color = Color("#6a8498")
