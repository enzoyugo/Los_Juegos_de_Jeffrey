...
func _ready() -> void:
	...
	var round_panel := _asset(root, HUD_ROUND, Vector2(750, -18), Vector2(420, 140))
	_wave = Layout.outlined_label("1", 52, ThemeRef.GOLD, HORIZONTAL_ALIGNMENT_CENTER)
	_wave.position = Vector2(145, 53)
	_wave.size = Vector2(130, 56)
	...
	var points_panel := _asset(root, HUD_POINTS, Vector2(18, -18), Vector2(420, 140))
	_points = Layout.outlined_label("0", 30, ThemeRef.GOLD, HORIZONTAL_ALIGNMENT_CENTER)
	_points.position = Vector2(178, 50)
	_points.size = Vector2(190, 44)
	...
	var health_panel := _asset(root, HUD_HEALTH, Vector2(18, 842), Vector2(520, 173))
	var hp_wrap := Control.new()
	_hp_wrap = hp_wrap
	root.add_child(hp_wrap)
	hp_wrap.position = Vector2(152, 113)
	hp_wrap.size = Vector2(300, 18)
	...
	_hp.position = Vector2(395, 72)
	_hp.size = Vector2(82, 42)
	health_panel.add_child(_hp)
	var weapon_panel := _asset(root, HUD_WEAPON, Vector2(1382, 842), Vector2(520, 173))
	_gun = Layout.outlined_label("PISTOLA", 21, ThemeRef.GOLD, HORIZONTAL_ALIGNMENT_LEFT)
	_gun.position = Vector2(92, 72)
	_gun.size = Vector2(230, 42)
	weapon_panel.add_child(_gun)
	_ammo = Layout.outlined_label("10 / 80", 28, ThemeRef.TEXT, HORIZONTAL_ALIGNMENT_CENTER)
	_ammo.position = Vector2(288, 72)
	_ammo.size = Vector2(190, 44)
