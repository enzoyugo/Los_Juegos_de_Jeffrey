class_name TrackCameraCandidateV2
extends Camera3D

## NON-CANONICAL camera tuning candidate. TrackMain / Config.CAM_* unchanged.
## HUMAN_REVIEW_REQUIRED. Do not treat as gameplay authority.

const Config := preload("res://scripts/track/track_config.gd")
const Telemetry := preload("res://scripts/track/track_debug_telemetry.gd")

const CAND_FOV_MIN := 72.0
const CAND_FOV_MAX := 90.0
const CAND_LOOK_AHEAD := 19.5
const CAND_YAW_LAG := 7.5
const CAND_FOLLOW := 16.0
const CAND_DISTANCE := 8.1
const CAND_HEIGHT := 2.45
...
func _physics_process(delta: float) -> void:
	...
	var look_ahead := lerpf(CAND_LOOK_AHEAD * 0.85, CAND_LOOK_AHEAD * 1.32, t)
	if airborne:
		look_ahead *= 1.12
	if road_look_dir.length() > 0.2:
		look_ahead *= 1.08
	var height := CAND_HEIGHT
	if airborne:
		height += 0.45
	var dist := CAND_DISTANCE + t * 0.8
	last_distance = dist
	...
	var fov_hi := CAND_FOV_MAX
	fov = lerpf(CAND_FOV_MIN, fov_hi, t)
