"""Shopping del Sol V3 — reference-driven facade/parking/interior.

Authority (studied, not memory):
- EXTERIOR_STATION_034 night: layered WHITE ARCH, gold 8-point star, SHOPPING del SOL,
  square-pier wings, warm uplights, zig-zag tan/terracotta plaza, palms.
- EXTERIOR_STATION_007 / 001: parking aisles, islands, zebra, curved-neck lamps.
- INTERIOR_STATION_032 / 035 + photos/shopping-del-sol.jpg: cream tiles with dark
  diamond accents, wide hall, high wood barrel vault.
...
BLEND = os.path.join(ROOT, "assets", "environments", "shopping_del_sol", "blender", "shopping_del_sol_zombies_environment_v3.blend")
EXPORT = os.path.join(ROOT, "assets", "environments", "shopping_del_sol", "blender", "exports", "shopping_del_sol_zombies_environment_v3.glb")
PROCESSED = os.path.join(ROOT, "assets", "environments", "shopping_del_sol", "processed", "shopping_del_sol_zombies_environment_v3.glb")
...
DECISIONS = os.path.join(ROOT, "docs", "generated", "sds_v3_asset_decisions.json")
AUTHORITY = os.path.join(ROOT, "docs", "generated", "sds_v3_authority_frames.txt")
...
def _facade(col, mats):
    z_face = 9.55
    _arch_segments(col, mats["cream"], 6.35, 0.55, 1.15, z_face, 0.15, 22, "ArchOuter")
    _arch_segments(col, mats["cream"], 5.85, 0.38, 0.9, z_face + 0.25, 0.2, 20, "ArchMid")
    _arch_segments(col, mats["cream"], 5.45, 0.28, 0.65, z_face + 0.45, 0.25, 18, "ArchInner")
    for i in range(14):
    ...
    bpy_util.box("DoorL", _s(1.55, 2.85, 0.1), _g(-0.85, 1.45, 8.9), (0, 0, 0), mats["glass"], col)
    bpy_util.box("DoorR", _s(1.55, 2.85, 0.1), _g(0.85, 1.45, 8.9), (0, 0, 0), mats["glass"], col)
    bpy_util.box("DoorFrame", _s(3.4, 3.05, 0.16), _g(0, 1.55, 8.84), (0, 0, 0), mats["metal"], col)
    bpy_util.box("DoorBar", _s(2.9, 0.07, 0.07), _g(0, 1.5, 8.98), (0, 0, 0), mats["metal"], col)
    bpy_util.box("PortalL", _s(1.15, 8.6, 2.2), _g(-5.7, 4.3, 8.5), (0, 0, 0), mats["cream"], col)
    bpy_util.box("PortalR", _s(1.15, 8.6, 2.2), _g(5.7, 4.3, 8.5), (0, 0, 0), mats["cream"], col)
    bpy_util.box("Canopy", _s(12.4, 0.22, 3.2), _g(0, 5.55, 10.2), (0, 0, 0), mats["cream"], col)
...
def _interior(col, mats):
    floor = bpy_util.box("HallFloor", _s(19.6, 0.08, 38.0), _g(0, 0.0, -10.5), (0, 0, 0), mats["tile"], col)
    _ensure_uv(floor, 2.2)
    bpy_util.box("HallWallL", _s(0.45, 9.6, 38.0), _g(-9.9, 4.8, -10.5), (0, 0, 0), mats["masonry"], col)
    bpy_util.box("HallWallR", _s(0.45, 9.6, 38.0), _g(9.9, 4.8, -10.5), (0, 0, 0), mats["masonry"], col)
    bpy_util.box("BrickBandL", _s(0.2, 3.2, 37.6), _g(-9.72, 7.4, -10.5), (0, 0, 0), mats["brick"], col)
    bpy_util.box("BrickBandR", _s(0.2, 3.2, 37.6), _g(9.72, 7.4, -10.5), (0, 0, 0), mats["brick"], col)
    for i, z in enumerate((4.0, -2.0, -8.0, -14.0, -20.0, -26.0)):
        _arch_segments(col, mats["wood"], 9.4, 0.22, 0.55, z, 2.4, 16, "VaultRib_%d" % i)
        bpy_util.box("ShopL_%d" % i, _s(5.8, 3.6, 0.1), _g(-6.8, 1.9, z - 2.4), (0, 0, 0), mats["glass"], col)
        bpy_util.box("ShopFrameL_%d" % i, _s(6.1, 3.85, 0.18), _g(-6.8, 1.95, z - 2.32), (0, 0, 0), mats["shop"], col)
        bpy_util.box("ShopR_%d" % i, _s(5.8, 3.6, 0.1), _g(6.8, 1.9, z - 2.4), (0, 0, 0), mats["glass"], col)
        bpy_util.box("ShopFrameR_%d" % i, _s(6.1, 3.85, 0.18), _g(6.8, 1.95, z - 2.32), (0, 0, 0), mats["shop"], col)
        bpy_util.box("AwningL_%d" % i, _s(5.4, 0.08, 0.85), _g(-6.8, 3.85, z - 1.9), (0, 0, 0), mats["terra"], col)
    ...
    bpy_util.box("VaultCap", _s(18.8, 0.18, 38.2), _g(0, 11.35, -10.5), (0, 0, 0), mats["wood"], col)
    bpy_util.box("Skylight", _s(6.4, 0.06, 28.0), _g(0, 11.48, -10.5), (0, 0, 0), mats["glass_in"], col)
    bpy_util.box("KioskBody", _s(2.8, 2.4, 2.8), _g(0, 1.2, -9.5), (0, 0, 0), mats["shop"], col)
    bpy_util.box("KioskTop", _s(3.05, 0.12, 3.05), _g(0, 2.45, -9.5), (0, 0, 0), mats["sign"], col)
    bpy_util.box("KioskGlass", _s(2.5, 1.4, 0.06), _g(0, 1.3, -8.08), (0, 0, 0), mats["glass"], col)
    bpy_util.box("HallBench", _s(2.2, 0.42, 0.55), _g(-2.6, 0.32, -4.5), (0, 0, 0), mats["bench"], col)
    bpy_util.box("Planter", _s(1.2, 0.55, 1.2), _g(3.0, 0.28, -4.6), (0, 0, 0), mats["curb"], col)
    bpy_util.ico("HallShrub", 0.62, _g(3.0, 1.0, -4.6), mats["fan"], col, 1)
    bpy_util.box("BackWall", _s(19.8, 9.8, 0.4), _g(0, 4.9, -29.6), (0, 0, 0), mats["masonry"], col)
    bpy_util.box("GalleryHint", _s(5.2, 3.4, 0.12), _g(0, 1.8, -29.35), (0, 0, 0), mats["dark"], col)
...
def _instance_cars(col):
    ...
    print("SDS_V3_CARS", [os.path.basename(p) for p in usable])
    if not usable:
        return 0
    templates = []
    for path in usable:
...
def _write_decisions(n_cars, psx_note):
    ...
    rows = [
        ...
        {"raw": "custom Blender", "decision": "USE", "why": "arch, star, sign, zig-zag plaza, vault, diamond tiles, piers, lamps, palms — library has none of these SDS identity pieces"},
    ]
...
def main():
    ...
    tile_path = os.path.join(TEX_DIR, "sds_interior_tile_v3.png")
    plaza_path = os.path.join(TEX_DIR, "sds_plaza_tile_v3.png")
    ...
    bpy_util.stats_report(EXPORT, {"asset": "shopping_del_sol_zombies_environment_v3"})
    ...
    print("SDS_ENV_V3_BUILT", EXPORT, "cars", n_cars)
