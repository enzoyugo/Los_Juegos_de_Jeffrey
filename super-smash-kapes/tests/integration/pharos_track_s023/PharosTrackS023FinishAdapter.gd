...
const AIRBORNE_RECOVERY_SECONDS := 1.0
const SAFE_FALL_Y := -2.0
...
func _physics_process(_delta: float) -> void:
	...
	var vertical_speed := _car.linear_velocity.y
	if vertical_speed < -5.0 or _car.global_position.y < -0.25:
		if _airborne_since < 0.0: _airborne_since = now
	else:
		_airborne_since = -1.0
	if _car.global_position.y < SAFE_FALL_Y:
		_finish("FAIL_FALL", "SAFE_HEIGHT")
		return
	if _airborne_since >= 0.0 and now - _airborne_since >= AIRBORNE_RECOVERY_SECONDS:
		if _recoveries < MAX_RECOVERIES:
			_recover("AIRBORNE_AFTER_CHECKPOINT_%d" % _checkpoint)
			_airborne_since = -1.0
		else:
			_finish("FAIL_ROUTE_DIVERGENCE", "AIRBORNE_RECOVERY_EXHAUSTED")
			return
	...
	# Diagnostic-only speed cap after the observed checkpoint-4 airborne event.
	# This changes controller input, not geometry or vehicle physics.
	var target_throttle := 0.38 if _checkpoint >= 3 else 0.72 if absf(steer) > 0.45 else 1.0
	_car.set("scripted_throttle", target_throttle)
	...
	_telemetry.append({"t":elapsed,"position":_car.global_position,"velocity":_car.linear_velocity,"speed":_car.linear_velocity.length(),"rotation":_car.global_rotation,"angular_velocity":_car.angular_velocity,"checkpoint":_checkpoint,"next_checkpoint":min(_checkpoint + 1, _route.size()),"nearest_route_index":nearest,"lateral_distance":sqrt(nearest_distance),"target":target,"steering":steer,"throttle":_car.get("scripted_throttle"),"brake":0.0,"recovery_state":_terminal_state,"height_over_track":_car.global_position.y,"recovery_attempts":_recoveries})
