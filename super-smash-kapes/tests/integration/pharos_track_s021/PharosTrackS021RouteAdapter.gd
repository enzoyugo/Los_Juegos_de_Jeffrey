...
const DEFAULT_OUT := "E:/PharosVisualization/experiments/track_generation/sprint_021_runtime_driver/runs"
...
func _ready() -> void:
	...
	if not _load_spec(): return _finish("FAIL_RUNTIME", "SPEC_LOAD")
	_load_glb_collision()
	_build_checkpoints()
	_spawn_car()
...
func _finish(result: String, failure: String) -> void:
	...
	var f := FileAccess.open("%s/%s_%s.json" % [_out_dir, _track_name, str(Time.get_ticks_msec())], FileAccess.WRITE)
	if f != null: f.store_string(JSON.stringify(row, "  "))
